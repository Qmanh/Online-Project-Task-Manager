...
const INVITATION_COLLECTION_SCHEMA = Joi.object({
  ...
  inviteId: Joi.string().required().pattern(OBJECT_ID_RULE).message(OBJECT_ID_RULE_MESSAGE),
...
const INVALID_UPDATE_FIELDS = ['_id', 'inviterId', 'inviteId', 'type', 'createdAt']
...
const createNewBoardInvitation = async (data) => {
  try {
    ...
    let newInvitationToAdd = {
      ...
      inviteId: new ObjectId(validData.inviteId)
    }
    ...
    const results = await GET_DB().collection(INVITATION_COLLECTION_NAME).aggregate([
      { $match: { $and: queryConditions } },
      {
        $lookup: {
          ...
          foreignField: '_id',
          as: 'inviter',
          //pipeline trong lookup de xu ly mot hoac nhieu luong can thiet
          //$project de chi dinh vai field khong muon lay ve bang cach gan bang 0
...
export const invitationModel = {
  createNewBoardInvitation, findOneById, update, findByUser
}